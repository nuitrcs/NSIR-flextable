...
rather than an HTML file. We will focus on creating tables in Word
documents, but `flextable` also supports creating tables in HTML and
PowerPoint.

You will learn how to use `flextable` to create tables customize various
aspects of the table such as merging cells, adding headers and footers,
and changing formatting.
...
Suppose we would like to center and bold all the headers. We can use the
`align()` function on the table.
...
You can use integers as an index of the cell for which you would like t
o select.
...
diamonds_ft %>%
  ...
  bg(i = 3, j = 3, bg = "skyblue", part = "body") %>%
...
```{r}
diamonds_ft %>%
  # bold and color cuts that are "Ideal" or "Premium"
